...
def _eager_init_llava_before_load_checkpoint(model):
    """Initialize lazy LLaVA/LoRA modules before loading checkpoint."""
    lazy_init_fn = getattr(model, '_lazy_init_llava', None)
    if lazy_init_fn is None:
        return
    try:
        inited = bool(lazy_init_fn())
    except Exception as exc:
        warnings.warn(f'Pre-checkpoint llava eager init failed: {exc}')
        return
    if not inited:
        runtime_error = getattr(model, '_llava_runtime_error', None)
        warnings.warn(
            f'LLaVA eager init before checkpoint returned False. '
            f'runtime_error={runtime_error}'
        )


def _save_test_run_metadata(cfg, args, jsonfile_prefix):
    """Save config and checkpoint metadata alongside formatted test results."""
    mmcv.mkdir_or_exist(jsonfile_prefix)

    dumped_config_path = osp.join(jsonfile_prefix, osp.basename(args.config))
    cfg.dump(dumped_config_path)

    metadata_path = osp.join(jsonfile_prefix, 'test_run_info.json')
    metadata = {
        'config_path': osp.abspath(args.config),
        'dumped_config_path': osp.abspath(dumped_config_path),
        'checkpoint_path': osp.abspath(args.checkpoint),
        'resolved_checkpoint_path': osp.realpath(args.checkpoint),
...
def main():
    ...
    _eager_init_llava_before_load_checkpoint(model)
    ...
    if rank == 0:
        ...
        _save_test_run_metadata(cfg, args, kwargs['jsonfile_prefix'])
        print(f'\nsaved test metadata to {kwargs["jsonfile_prefix"]}')
        if args.format_only:
