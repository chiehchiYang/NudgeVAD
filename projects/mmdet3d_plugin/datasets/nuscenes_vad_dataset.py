...
class VADCustomNuScenesDataset(NuScenesDataset):
    ...
    """
    MAPCLASSES = ('divider',)
    DRIVE_QA_MODES = ('normal', 'drivelm', 'drivelm_subset')
    ...
    def __init__(
        ...
        drive_qa=False,
        drive_qa_ann_file=None,
        drive_qa_mode='normal',
        drive_qa_subset_file=None,
        doscenes_anchor_only=False,
        # If set (list of frame_idx, e.g. [3,4,5]), overrides
        # doscenes_anchor_only and filters visible samples to those frames.
        # Use to widen anchor-only training around the anchor (frame_idx=4)
        # without going all the way to full 28k.
        ...
        **kwargs
        ...
        self.drive_qa_mode = self._normalize_drive_qa_mode(drive_qa_mode)
        self.drive_qa_subset_file = drive_qa_subset_file
        self.full_data_infos = list(self.data_infos)
        self.full_flag = np.array(self.flag, copy=True) if hasattr(self, 'flag') else None
        self.visible_to_full_data_idx = list(range(len(self.full_data_infos)))
        self.anchor_sample_tokens = {
            info['token'] for info in self.full_data_infos if isinstance(info.get('token'), str)
        }
        if not self.test_mode and self.drive_qa_mode in ('drivelm', 'drivelm_subset'):
            subset_tokens = self._load_subset_sample_tokens(drive_qa_subset_file)
            self._apply_anchor_subset(subset_tokens)
        elif doscenes_anchor_frames is not None:
            # Anchor-window training: keep samples whose frame_idx is in the
            # provided list. Lets us add ±N neighbors of the anchor for 3-5x
            # more training signal while keeping the distribution near anchor.
            frames = set(int(f) for f in doscenes_anchor_frames)
            anchor_tokens = {
                info['token']
                for info in self.full_data_infos
                if info.get('frame_idx') in frames and isinstance(info.get('token'), str)
            }
            print(f'[VADCustomNuScenesDataset] doscenes_anchor_frames={sorted(frames)}: '
                  f'{len(anchor_tokens)} samples out of {len(self.full_data_infos)}')
            self._apply_anchor_subset(anchor_tokens)
        elif doscenes_anchor_only:
            # doScenes leaderboard methodology: only frame_idx==4 (5th keyframe
            # of each scene) is the prediction anchor. Filter visible samples
            # to those anchors. The full pkl is still kept in `full_data_infos`
            # so the temporal BEV queue (`queue_length` prev frames) keeps
            # working — `prepare_train_data` walks `full_data_infos` for
            # earlier frames in the same scene.
            anchor_tokens = {
                info['token']
                for info in self.full_data_infos
                if info.get('frame_idx') == 4 and isinstance(info.get('token'), str)
            }
            print(f'[VADCustomNuScenesDataset] doscenes_anchor_only=True: '
                  f'{len(anchor_tokens)} anchors out of {len(self.full_data_infos)} samples')
            self._apply_anchor_subset(anchor_tokens)
        else:
            self.data_infos = list(self.full_data_infos)
            if self.full_flag is not None:
                self.flag = np.array(self.full_flag, copy=True)
            elif not hasattr(self, 'flag'):
                self.flag = np.zeros(len(self.data_infos), dtype=np.uint8)

        self.drive_qa = bool(drive_qa or self.drive_qa_mode != 'normal')
        self.drive_qa_ann_file = drive_qa_ann_file
        if self.drive_qa:
            if self.drive_qa_mode == 'drivelm':
                self.drive_qa_by_token = self._load_drivelm_drive_qa_annotations(
                    drive_qa_subset_file, valid_tokens=self.anchor_sample_tokens
                )
            else:
                valid_tokens = self.anchor_sample_tokens if self.drive_qa_mode == 'drivelm_subset' else None
                self.drive_qa_by_token = self._load_drive_qa_annotations(
                    drive_qa_ann_file, valid_tokens=valid_tokens
                )
        else:
            self.drive_qa_by_token = {}

    def _normalize_drive_qa_mode(self, drive_qa_mode: Optional[str]) -> str:
        mode = 'normal' if drive_qa_mode is None else str(drive_qa_mode).strip().lower().replace(' ', '_')
        if mode not in self.DRIVE_QA_MODES:
            raise ValueError(
                f'Unsupported drive_qa_mode={drive_qa_mode!r}. '
                f'Expected one of {self.DRIVE_QA_MODES}.'
            )
        return mode

    def _iter_drivelm_key_frames(self, subset_path: Optional[str]):
        if subset_path is None:
            raise ValueError(
                'drive_qa_subset_file must be provided when drive_qa_mode is '
                '"drivelm" or "drivelm_subset".'
            )

        subset_path = osp.expanduser(subset_path)
        if not osp.exists(subset_path):
            raise FileNotFoundError(f'DriveLM subset file not found: {subset_path}')
        ...
        if not isinstance(payload, dict):
            raise ValueError(
                f'Expected DriveLM subset file {subset_path} to decode to dict, got {type(payload)}.'
            )

        for scene_token, scene_payload in payload.items():
            if not isinstance(scene_payload, dict):
                continue
            key_frames = scene_payload.get('key_frames', None)
            if not isinstance(key_frames, dict):
                continue
            for sample_token, frame_payload in key_frames.items():
                if not isinstance(sample_token, str):
                    continue
                if not isinstance(frame_payload, dict):
                    frame_payload = {}
                yield scene_token, sample_token, frame_payload

    def _load_subset_sample_tokens(self, subset_path: Optional[str]) -> Set[str]:
        sample_tokens = {
            sample_token for _, sample_token, _ in self._iter_drivelm_key_frames(subset_path)
        }
        if len(sample_tokens) == 0:
            raise ValueError(f'No sample tokens found in DriveLM subset file: {subset_path}')
        return sample_tokens

    def _apply_anchor_subset(self, sample_tokens: Set[str]) -> None:
        visible_to_full = [
            idx for idx, info in enumerate(self.full_data_infos)
            if info.get('token') in sample_tokens
        ]
        if len(visible_to_full) == 0:
            raise ValueError('DriveLM subset filtering removed all training samples.')

        self.visible_to_full_data_idx = visible_to_full
        self.data_infos = [self.full_data_infos[idx] for idx in visible_to_full]
        self.anchor_sample_tokens = {
            info['token'] for info in self.data_infos if isinstance(info.get('token'), str)
        }
        if self.full_flag is not None:
            self.flag = np.array(self.full_flag[visible_to_full], copy=True)
        else:
            self.flag = np.zeros(len(self.data_infos), dtype=np.uint8)

    def _iter_drive_qa_rows(self, qa_path: str):
        with open(qa_path, 'r', encoding='utf-8') as f:
            for line_no, line in enumerate(f, start=1):
                ...
                try:
                    ...
                    raise ValueError(
                        f'Invalid DriveQA JSONL row at line {line_no} in {qa_path}: {exc}'
                    ) from exc
                ...
                if isinstance(payload, dict):
                    ...
                    for row in payload:
                        if not isinstance(row, dict):
                            raise ValueError(
                                f'Expected DriveQA JSONL row to decode to dict/list[dict], got {type(row)}.'
                            )
                        yield row
                else:
                    raise ValueError(
                        f'Expected DriveQA JSONL row to decode to dict/list[dict], got {type(payload)}.'
                    )

    @staticmethod
    def _normalize_drive_qa_group(qa_group: Any) -> Optional[List[Dict[str, str]]]:
        if not isinstance(qa_group, list):
            return None
        ...
        for qa_item in qa_group:
            ...
            if not isinstance(question, str) or not isinstance(answer, str):
                continue
            normalized.append(dict(Q=question, A=answer))

        return normalized or None

    def _load_drivelm_drive_qa_annotations(
        self,
        subset_path: Optional[str],
        valid_tokens: Optional[Set[str]] = None,
    ) -> Dict[str, Any]:
        valid_token_set = set(valid_tokens) if valid_tokens is not None else None
        category_map = {
        ...
        drive_qa_by_token = {}
        for _, sample_token, frame_payload in self._iter_drivelm_key_frames(subset_path):
            if valid_token_set is not None and sample_token not in valid_token_set:
                continue
            qa_payload = frame_payload.get('QA', None)
            if not isinstance(qa_payload, dict):
                continue

            normalized_pairs = {}
            for source_category, target_category in category_map.items():
                normalized_group = self._normalize_drive_qa_group(qa_payload.get(source_category, None))
                if normalized_group is not None:
                    normalized_pairs[target_category] = normalized_group

            if normalized_pairs:
                drive_qa_by_token[sample_token] = normalized_pairs

        return drive_qa_by_token

    def _load_drive_qa_annotations(
        self,
        qa_path: Optional[str],
        valid_tokens: Optional[Set[str]] = None,
    ) -> Dict[str, Any]:
        if qa_path is None:
            raise ValueError('drive_qa_ann_file must be provided when drive_qa=True.')

        qa_path = osp.expanduser(qa_path)
        if not osp.exists(qa_path):
            raise FileNotFoundError(f'DriveQA annotation file not found: {qa_path}')

        valid_token_set = set(valid_tokens) if valid_tokens is not None else None
        drive_qa_by_token = {}
        for row in self._iter_drive_qa_rows(qa_path):
            token = row.get('token')
            if not isinstance(token, str):
                continue
            if valid_token_set is not None and token not in valid_token_set:
                continue
            if row.get('qa_pairs') is None:
                continue
            drive_qa_by_token[token] = row['qa_pairs']
        return drive_qa_by_token

    def _attach_drive_qa_to_metas(self, example, input_dict):
        if not self.drive_qa or example is None:
            return example
        img_metas = example.get('img_metas', None)
        if img_metas is None:
            return example
        drive_qa = input_dict.get('drive_qa', None)
        self._set_drive_qa_on_img_metas(img_metas, drive_qa)
        return example

    def _set_drive_qa_on_img_metas(self, img_metas, drive_qa):
        if img_metas is None:
            return False

        if hasattr(img_metas, 'data'):
            return self._set_drive_qa_on_img_metas(img_metas.data, drive_qa)

        if isinstance(img_metas, dict):
            img_metas['drive_qa'] = drive_qa
            img_metas['has_drive_qa'] = drive_qa is not None
            return True

        if isinstance(img_metas, (list, tuple)):
            updated = False
            for item in img_metas:
                updated = self._set_drive_qa_on_img_metas(item, drive_qa) or updated
            return updated
    ...
    def prepare_train_data(self, index):
        ...
        """
        data_queue = []
        full_index = self.visible_to_full_data_idx[index]

        # temporal aug
        prev_indexs_list = list(range(full_index-self.queue_length, full_index))
        random.shuffle(prev_indexs_list)
        prev_indexs_list = sorted(prev_indexs_list[1:], reverse=True)
        ##

        input_dict = self.get_full_data_info(full_index)
        ...
        example = self._attach_drive_qa_to_metas(example, input_dict)
        ...
        for i in prev_indexs_list:
            i = max(0, i)
            input_dict = self.get_full_data_info(i)
            if input_dict is None:
                return None
    ...
    def _build_ann_info_from_info(self, info):
        """Build annotation info from a single info record."""
    ...
    def get_ann_info(self, index):
        ...
        """
        return self._build_ann_info_from_info(self.data_infos[index])

    def get_full_ann_info(self, full_index):
        """Get annotation info from the unfiltered full dataset index."""
        return self._build_ann_info_from_info(self.full_data_infos[full_index])

    def _build_input_dict_from_info(self, info, annos):
        """Build the model input dict from a single info record."""
        ...
        if self.drive_qa:
            input_dict['drive_qa'] = self.drive_qa_by_token.get(info['token'], None)
    ...
    def get_data_info(self, index):
        ...
        return self._build_input_dict_from_info(info, annos)

    def get_full_data_info(self, full_index):
        """Get data info from the unfiltered full dataset index."""
        info = self.full_data_infos[full_index]
        annos = self.get_full_ann_info(full_index)
        return self._build_input_dict_from_info(info, annos)
    ...
    def _format_bbox(self, results, jsonfile_prefix=None, score_thresh=0.2):
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            ...
            if 'ego_fut_preds_vision' in det:
                plan_annos_vision[sample_token] = [det['ego_fut_preds_vision'], det['ego_fut_cmd']]
            if 'ego_fut_preds_llava' in det:
                plan_annos_mllm[sample_token] = [det['ego_fut_preds_llava'], det['ego_fut_cmd']]
        ...
        if len(plan_annos_vision) > 0:
            nusc_submissions['plan_results_vision'] = plan_annos_vision
        if len(plan_annos_mllm) > 0:
            nusc_submissions['plan_results_mllm'] = plan_annos_mllm
