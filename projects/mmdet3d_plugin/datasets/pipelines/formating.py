...
class CustomDefaultFormatBundle3D(DefaultFormatBundle3D):
    ...
    def __call__(self, results):
        ...
        if self.with_ego:
            ...
            if 'llava_mosaic_img' in results and results['llava_mosaic_img'] is not None:
                llava_mosaic_img = np.ascontiguousarray(results['llava_mosaic_img'].transpose(2, 0, 1))
                results['llava_mosaic_img'] = DC(to_tensor(llava_mosaic_img), stack=True)
            elif 'llava_mosaic_img' in results:
                results.pop('llava_mosaic_img')
            if 'drive_qa_samples' in results:
                results['drive_qa_samples'] = DC(results['drive_qa_samples'], cpu_only=True)
