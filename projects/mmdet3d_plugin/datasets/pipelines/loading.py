...
class LoadVADImgCache(object):
    """Multi-view image loader compatible with VAD's encoder.

    The original work used a custom image-cache transform (registered by an
    internal qwen3vl plugin) that pre-rendered the 6-cam stack to disk and
    set ``img_shape`` as a *list of per-view tuples*. That plugin is not
    released; for repo reproducibility we provide this drop-in replacement
    which:

      1. Loads the 6 camera images from ``img_filename`` (same as
         ``LoadMultiViewImageFromFiles``).
      2. Resizes them to ``scale`` × original resolution (default 0.4 →
         640×360 from 1600×900). This matches the resolution the test
         pipeline uses (``RandomScaleImageMultiViewImage(scales=[0.4])``)
         and keeps activation memory in line with the 8 × 24 GB training
         budget of the original run.
      3. Pads to ``size_divisor`` multiples on H and W.
      4. Expands ``img_shape`` / ``ori_shape`` / ``pad_shape`` /
         ``scale_factor`` into per-view lists so VAD's BEV encoder can
         index ``img_shape[cam][1]`` to get the per-camera width.

    Args:
        cache_dir (str, optional): kept for config-file compatibility; ignored.
        to_float32 (bool): forwarded to ``LoadMultiViewImageFromFiles``.
        scale (float): pre-encoder resize ratio. Default 0.4.
        size_divisor (int): pad H/W up to a multiple of this. Default 32.
    """

    def __init__(self, cache_dir=None, to_float32=True, color_type='unchanged',
                 scale=0.4, size_divisor=32):
        self._loader = LoadMultiViewImageFromFiles(
            to_float32=to_float32, color_type=color_type)
        self.scale = float(scale)
        self.size_divisor = int(size_divisor)

    def _resize_and_pad(self, img):
        """Resize ``img`` (H, W, 3) by self.scale then pad to size_divisor."""
        h, w = img.shape[:2]
        new_h, new_w = int(round(h * self.scale)), int(round(w * self.scale))
        resized = mmcv.imresize(img, (new_w, new_h), return_scale=False)
        pad_h = int(np.ceil(new_h / self.size_divisor) * self.size_divisor)
        pad_w = int(np.ceil(new_w / self.size_divisor) * self.size_divisor)
        if pad_h == new_h and pad_w == new_w:
            return resized, (new_h, new_w, resized.shape[2])
        padded = np.zeros((pad_h, pad_w, resized.shape[2]), dtype=resized.dtype)
        padded[:new_h, :new_w] = resized
        return padded, (pad_h, pad_w, resized.shape[2])

    def __call__(self, results):
        results = self._loader(results)
        img = results.get('img')
        if img is None:
            return results

        # Resize + pad each camera view
        if isinstance(img, list):
            resized_imgs, per_view_shape = [], None
            for v in img:
                r, shp = self._resize_and_pad(v)
                resized_imgs.append(r)
                per_view_shape = shp
            results['img'] = resized_imgs
            n_views = len(resized_imgs)
        else:  # stacked array, shape (H, W, 3, N) or (H, W, 3)
            arr = np.asarray(img)
            if arr.ndim == 4:
                resized_views = []
                for i in range(arr.shape[-1]):
                    r, shp = self._resize_and_pad(arr[..., i])
                    resized_views.append(r)
                results['img'] = np.stack(resized_views, axis=-1)
                per_view_shape = shp
                n_views = arr.shape[-1]
            else:
                r, shp = self._resize_and_pad(arr)
                results['img'] = r
                per_view_shape = shp
                n_views = 1

        # Update meta: per-view tuples (list) — VAD's encoder indexes [cam][1]
        results['img_shape'] = [tuple(per_view_shape)] * n_views
        results['ori_shape'] = [tuple(per_view_shape)] * n_views
        results['pad_shape'] = [tuple(per_view_shape)] * n_views
        results['scale_factor'] = [self.scale] * n_views

        # Scale intrinsic camera matrices to match the resized image size.
        # `lidar2img` / `cam_intrinsic` must reflect the new pixel coordinates.
        for key in ('lidar2img', 'cam_intrinsic'):
            mats = results.get(key)
            if mats is None:
                continue
            new_mats = []
            for m in mats:
                m = np.array(m, dtype=np.float64)
                # First two rows of the projection map to pixel (u, v) → scale.
                m[:2, :] *= self.scale
                new_mats.append(m)
            results[key] = new_mats
        return results
