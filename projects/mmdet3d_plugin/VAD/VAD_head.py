...
class EgoGRUDecoder(nn.Module):
    """TransFuser++-style autoregressive GRU decoder for ego planning.

    Replaces the single-MLP 12-step prediction (which suffers super-linear
    error compounding past t=3 s) with a GRUCell that produces (path_dir,
    speed_logits) per step and feeds the composed offset back into the
    next step.

    Per step t:
      x_t      = [ego_feats_const, prev_offset_flat]
      h_t      = GRUCell(input_proj(x_t), h_{t-1})
      path_t   = path_proj(h_t)            [B, M, 2]   raw direction
      speed_t  = speed_proj(h_t)           [B, M, K]   bin logits
      compose  = (path_t / |path_t|) · E[speed]_softmax · dt
      prev     = compose                  (autoregressive feedback)
    """

    def __init__(self, ego_feat_dim, hidden_dim, fut_ts, num_modes,
                 num_speed_bins, dt, bin_centres):
        super().__init__()
        self.fut_ts = int(fut_ts)
        self.num_modes = int(num_modes)
        self.num_speed_bins = int(num_speed_bins)
        self.dt = float(dt)
        self.register_buffer('_bin_centres', torch.as_tensor(bin_centres,
                                                             dtype=torch.float32))
        self.init_h     = nn.Linear(ego_feat_dim, hidden_dim)
        self.input_proj = nn.Linear(ego_feat_dim + num_modes * 2, hidden_dim)
        self.gru        = nn.GRUCell(hidden_dim, hidden_dim)
        self.out_path   = nn.Linear(hidden_dim, num_modes * 2)
        self.out_speed  = nn.Linear(hidden_dim, num_modes * num_speed_bins)

    def forward(self, ego_feats):
        """ego_feats: [B, ego_feat_dim] → (path[B,M,T,2], speed[B,M,T,K])"""
        B = ego_feats.shape[0]
        h = torch.tanh(self.init_h(ego_feats))
        prev = torch.zeros(B, self.num_modes * 2,
                           device=ego_feats.device, dtype=ego_feats.dtype)
        bin_c = self._bin_centres.to(ego_feats.dtype)
        path_steps, speed_steps = [], []
        for _ in range(self.fut_ts):
            x = torch.cat([ego_feats, prev], dim=-1)
            x = self.input_proj(x)
            h = self.gru(x, h)
            path_t  = self.out_path(h).reshape(B, self.num_modes, 2)
            speed_t = self.out_speed(h).reshape(B, self.num_modes, self.num_speed_bins)
            path_steps.append(path_t)
            speed_steps.append(speed_t)
            # Compose offset for next step's autoregressive input.
            path_norm = torch.norm(path_t, dim=-1, keepdim=True).clamp(min=1e-3)
            path_dir  = path_t / path_norm
            speed_p   = F.softmax(speed_t, dim=-1)
            speed_v   = (speed_p * bin_c).sum(-1, keepdim=True)
            offset    = path_dir * speed_v * self.dt           # [B, M, 2]
            prev      = offset.reshape(B, -1)
        path  = torch.stack(path_steps,  dim=2)        # [B, M, T, 2]
        speed = torch.stack(speed_steps, dim=2)        # [B, M, T, K]
        return path, speed


class TrajRefinementGRU(nn.Module):
    """Post-hoc per-step trajectory refinement.

    Takes the single-MLP raw 12-step (per-mode) offset prediction plus
    ego_feats and runs a 1-layer GRU over the time dimension. Output is a
    DELTA added to the raw prediction (residual connection), so the module
    only needs to learn corrections — the "default" remains v9c's MLP.

    Goal: introduce inter-step coupling so step t can see info from steps
    1..t-1 (forward-only GRU) — this is what addresses the super-linear
    error compounding observed in v9c without paying the decouple/speed-bin
    penalty seen in v9e/v9f/v9g/v9h.

    Per step t input:
      [raw_offset_t (M*2), ego_feats_proj (64), step_idx_emb (32)]
    """
    ...
    def __init__(self, ego_feat_dim, hidden_dim, fut_ts, num_modes):
        ...
        self.fut_ts = int(fut_ts)
        self.num_modes = int(num_modes)
        self.step_emb = nn.Embedding(fut_ts, 32)
        self.ego_proj = nn.Linear(ego_feat_dim, 64)
        in_dim = num_modes * 2 + 64 + 32
        self.input_proj = nn.Linear(in_dim, hidden_dim)
        self.gru = nn.GRU(hidden_dim, hidden_dim, batch_first=True)
        self.out_proj = nn.Linear(hidden_dim, num_modes * 2)
        # Zero-init out_proj so the residual delta starts at 0 → at init,
        # refinement is identity → can only HELP, not hurt, in early epochs.
        nn.init.zeros_(self.out_proj.weight)
        nn.init.zeros_(self.out_proj.bias)

    def forward(self, raw_offsets, ego_feats):
        """raw_offsets: [B, M, T, 2]   ego_feats: [B, F] or [B, 1, F]"""
        B, M, T, _ = raw_offsets.shape
        if ego_feats.ndim == 3:
            ego_feats = ego_feats.squeeze(1)
        # [B, T, M*2]
        flat = raw_offsets.permute(0, 2, 1, 3).reshape(B, T, M * 2)
        ego_p = self.ego_proj(ego_feats).unsqueeze(1).expand(-1, T, -1)   # [B, T, 64]
        step_idx = torch.arange(T, device=ego_feats.device).unsqueeze(0).expand(B, -1)
        step_e = self.step_emb(step_idx)                                  # [B, T, 32]
        x = torch.cat([flat, ego_p, step_e], dim=-1)
        x = self.input_proj(x)
        out, _ = self.gru(x)                                              # [B, T, H]
        delta = self.out_proj(out).reshape(B, T, M, 2).permute(0, 2, 1, 3)  # [B, M, T, 2]
        return raw_offsets + delta
...
class VADHead(DETRHead):
    ...
    def __init__(self,
                ...
                loss_map_cls=dict(
                 ...
                 valid_fut_ts=6,
                 # === Auxiliary speed-class head (TransFuser++ inspired) ===
                 enable_speed_head=False,
                 speed_bins=(0.5, 2.0, 5.0, 10.0, 15.0, 20.0),  # 6 thresholds → 7 classes
                 speed_dt=0.5,            # h4f12 step interval (s)
                 loss_speed_weight=0.1,
                 # === v9e: TransFuser++ path / speed decouple ===
                 # When True, ego_fut_decoder output is interpreted as a
                 # PATH DIRECTION (normalised to unit vectors per step), and
                 # speed magnitude comes from softmax(speed_logits) · bin_centres.
                 # Final per-step offset = unit_direction × speed × dt.
                 # Requires enable_speed_head=True (we re-use the speed CE head).
                 enable_path_speed_decouple=False,
                 # === v9f: TransFuser++ GRU autoregressive path decoder ===
                 # Replaces single-MLP 12-step prediction with a GRUCell that
                 # rolls out 12 steps autoregressively. At each step the
                 # composed offset (direction × speed × dt) feeds back as
                 # input to the next, providing temporal context that single-MLP
                 # lacks. Implies enable_path_speed_decouple semantically.
                 enable_gru_decoder=False,
                 gru_hidden_dim=256,
                 # === v9j: post-hoc trajectory refinement (forward-only BiGRU) ===
                 # Adds a residual delta on top of the single-MLP path output.
                 # Doesn't replace anything — just lets later steps see earlier
                 # steps' predictions. Speed CE stays as aux. Init delta=0
                 # so at init the refinement is identity.
                 enable_traj_refinement=False,
                 traj_refine_hidden_dim=128,
                 # === v9k: v0 skip-channel into speed head ===
                 # Pipes ego_lcf_feat[..., 7] = v0 (current longitudinal speed,
                 # m/s) directly through a small MLP into a per-step per-bin
                 # bias added to speed_logits. Zero-init last layer so v9k
                 # starts identical to v9c at init; gradient pulls speed head
                 # toward "future speed close to current speed" automatically.
                 # Targets the failure mode where v9c under-predicts speed in
                 # high-speed cruise scenes (single-MLP speed head ignores
                 # the v0 already buried in 512-d ego_feats).
                 enable_v0_skip=False,
                 v0_skip_hidden_dim=32,
                 v0_lcf_idx=7,
                 # === v7: soft cmd-as-input routing ===
                 # When True, decouple cmd from per-mode hard selection:
                 # cmd one-hot → small Linear embed → concat into ego_feats,
                 # ego_fut_decoder + ego_speed_decoder output a SINGLE
                 # trajectory (no mode dim), and loss / inference no longer
                 # use cmd as mode-mask / argmax-pick. This makes cmd a
                 # soft conditioning signal — wrong cmd no longer
                 # catastrophically picks the wrong mode.
                 enable_soft_cmd_routing=False,
                 cmd_emb_dim=32,
                 # === Long-horizon emphasis (FDE-style) ===
                 # Linear ramp on per-step planning L1 weight. 1.0 = uniform
                 # (default, original VAD behavior). e.g. 2.0 means weight
                 # ramps 1.0 → 2.0 from step 0 to step T-1 → late-horizon
                 # errors weigh more. Helps push 4-6s ADE without changing
                 # eval metric.
                 plan_step_weight_end=1.0,
                 # Extra L1 loss on the final timestep (FDE-style term) added
                 # on top of regular plan_reg loss. The weight multiplies
                 # loss_plan_reg.loss_weight, so 0.5 = half the regular per-
                 # step weight. 0 disables.
                 loss_plan_fde_weight=0.0,
                 **kwargs):
        ...
        # FDE-style emphasis (see kwargs above for semantics)
        self.plan_step_weight_end = float(plan_step_weight_end)
        self.loss_plan_fde_weight = float(loss_plan_fde_weight)
        # Speed-class head config (built lazily after embed_dims known in _init_layers)
        self.enable_speed_head = enable_speed_head
        # speed_bins thresholds → num_speed_bins = len(thresholds) + 1 classes
        self.speed_bins = tuple(speed_bins) if speed_bins is not None else tuple()
        self.num_speed_bins = len(self.speed_bins) + 1 if self.enable_speed_head else 0
        self.speed_dt = float(speed_dt)
        self.loss_speed_weight = float(loss_speed_weight)
        # Soft cmd routing config
        self.enable_soft_cmd_routing = bool(enable_soft_cmd_routing)
        self.cmd_emb_dim = int(cmd_emb_dim) if self.enable_soft_cmd_routing else 0
        # Path / speed decouple (v9e)
        self.enable_path_speed_decouple = bool(enable_path_speed_decouple)
        if self.enable_path_speed_decouple:
            assert self.enable_speed_head, (
                'enable_path_speed_decouple=True requires enable_speed_head=True '
                '(speed CE head provides the magnitude via softmax expected value)')
        # v9f: GRU autoregressive decoder
        self.enable_gru_decoder = bool(enable_gru_decoder)
        self.gru_hidden_dim = int(gru_hidden_dim)
        # v9j: post-hoc refinement
        self.enable_traj_refinement = bool(enable_traj_refinement)
        self.traj_refine_hidden_dim = int(traj_refine_hidden_dim)
        # v9k: v0 skip channel into speed head
        self.enable_v0_skip = bool(enable_v0_skip)
        self.v0_skip_hidden_dim = int(v0_skip_hidden_dim)
        self.v0_lcf_idx = int(v0_lcf_idx)
        if self.enable_v0_skip:
            assert self.enable_speed_head, (
                'enable_v0_skip=True requires enable_speed_head=True')
        if self.enable_gru_decoder:
            assert self.enable_speed_head, (
                'enable_gru_decoder=True requires enable_speed_head=True '
                '(GRU emits speed_logits per step; needs bin definition)')
            # Force decouple flag so bin_centres buffer is built
            self.enable_path_speed_decouple = True
    ...
    def _init_layers(self):
        ...
        ego_fut_dec_base_dim = self.embed_dims*2 + len(self.ego_lcf_feat_idx) \
            if self.ego_lcf_feat_idx is not None else self.embed_dims*2
        # v7: cmd embedding gets concat'd into ego_feats before the planner MLP
        if self.enable_soft_cmd_routing:
            self.cmd_embed = Linear(self.ego_fut_mode, self.cmd_emb_dim)
            ego_fut_dec_in_dim = ego_fut_dec_base_dim + self.cmd_emb_dim
            # single-trajectory output (mode dim = 1 kept for downstream code compat)
            ego_fut_out_dim = self.fut_ts * 2
            ego_speed_out_dim = self.fut_ts * self.num_speed_bins
        else:
            ego_fut_dec_in_dim = ego_fut_dec_base_dim
            ego_fut_out_dim = self.ego_fut_mode * self.fut_ts * 2
            ego_speed_out_dim = self.ego_fut_mode * self.fut_ts * self.num_speed_bins

        # When GRU decoder is on, it REPLACES the MLP path/speed heads.
        # Skip building those MLPs to avoid DDP "unused-params" error.
        if not self.enable_gru_decoder:
            ego_fut_decoder = []
            for _ in range(self.num_reg_fcs):
                ego_fut_decoder.append(Linear(ego_fut_dec_in_dim, ego_fut_dec_in_dim))
                ego_fut_decoder.append(nn.ReLU())
            ego_fut_decoder.append(Linear(ego_fut_dec_in_dim, ego_fut_out_dim))
            self.ego_fut_decoder = nn.Sequential(*ego_fut_decoder)

        # Auxiliary speed-class decoder (parallel to ego_fut_decoder).
        # Outputs per-step speed-bin logits; trained with CE,
        # provides extra long-horizon supervision (TransFuser++ style).
        if self.enable_speed_head and not self.enable_gru_decoder:
            ego_speed_dec = []
            for _ in range(2):
                ego_speed_dec.append(Linear(ego_fut_dec_in_dim, ego_fut_dec_in_dim))
                ego_speed_dec.append(nn.ReLU())
            ego_speed_dec.append(Linear(ego_fut_dec_in_dim, ego_speed_out_dim))
            self.ego_speed_decoder = nn.Sequential(*ego_speed_dec)

        # bin_centres buffer is needed by GRU regardless of MLP build path.
        if self.enable_speed_head:

            # Pre-compute speed-bin centres as a buffer for path/speed decouple.
            # cls 0 spans [0, t0]    → centre = t0/2
            # cls i spans [t_{i-1}, t_i] → centre = midpoint
            # cls N-1 spans [t_{N-2}, ∞) → centre = t_{N-2} + 2.5  (open-ended pad)
            if self.enable_path_speed_decouple:
                t = list(self.speed_bins)
                centres = [t[0] / 2.0]
                for i in range(len(t) - 1):
                    centres.append((t[i] + t[i + 1]) / 2.0)
                centres.append(t[-1] + 2.5)
                self.register_buffer(
                    '_decouple_bin_centres',
                    torch.tensor(centres, dtype=torch.float32))

            # v9k: v0 skip channel — small MLP from v0 (1-d) → per-step per-bin
            # bias added to speed_logits. Last layer zero-init so the residual
            # is identity at start (no behavioural change vs v9c at init).
            if self.enable_v0_skip:
                v0_proj = nn.Sequential(
                    nn.Linear(1, self.v0_skip_hidden_dim),
                    nn.ReLU(),
                    nn.Linear(self.v0_skip_hidden_dim, self.fut_ts * self.num_speed_bins),
                )
                nn.init.zeros_(v0_proj[-1].weight)
                nn.init.zeros_(v0_proj[-1].bias)
                self.speed_v0_proj = v0_proj

        # v9f: GRU autoregressive path decoder.
        # This *replaces* the single-MLP path/speed decoders functionally —
        # we still build the MLP heads above (they become dead weight when
        # GRU is on) for code simplicity. GRU gets the same `ego_fut_dec_in_dim`
        # input as the MLP path would have.
        # v9j: trajectory refinement BiGRU (orthogonal to GRU decoder)
        if self.enable_traj_refinement:
            self.ego_traj_refiner = TrajRefinementGRU(
                ego_feat_dim=ego_fut_dec_in_dim,
                hidden_dim=self.traj_refine_hidden_dim,
                fut_ts=self.fut_ts,
                num_modes=(1 if self.enable_soft_cmd_routing else self.ego_fut_mode),
            )

        if self.enable_gru_decoder:
            self.ego_gru_decoder = EgoGRUDecoder(
                ego_feat_dim=ego_fut_dec_in_dim,
                hidden_dim=self.gru_hidden_dim,
                fut_ts=self.fut_ts,
                num_modes=(1 if self.enable_soft_cmd_routing else self.ego_fut_mode),
                num_speed_bins=self.num_speed_bins,
                dt=self.speed_dt,
                bin_centres=self._decouple_bin_centres,
            )
    ...
    def forward(self,
        ...
        # v7 soft routing: concat cmd embedding to ego_feats so the planner
        # sees navigation intent as an input feature, not just a mode mask.
        if self.enable_soft_cmd_routing:
            assert ego_fut_cmd is not None, (
                'enable_soft_cmd_routing=True requires ego_fut_cmd to be passed '
                'into VADHead.forward()')
            # squeeze potential extra leading dims down to [B, ego_fut_mode]
            cmd_in = ego_fut_cmd
            while cmd_in.dim() > 2:
                cmd_in = cmd_in.squeeze(1)
            cmd_in = cmd_in.to(ego_feats.dtype)             # [B, ego_fut_mode]
            cmd_emb = self.cmd_embed(cmd_in).unsqueeze(1)    # [B, 1, cmd_emb_dim]
            ego_feats = torch.cat([ego_feats, cmd_emb], dim=-1)

        # === v9f: GRU autoregressive decoder ===
        if self.enable_gru_decoder:
            ego_feats_in = ego_feats.squeeze(1)         # [B, F]
            path_raw, speed_logits = self.ego_gru_decoder(ego_feats_in)  # [B, M, T, 2], [B, M, T, K]
            # Compose final per-step offsets — same as decouple
            path_norm = torch.norm(path_raw, dim=-1, keepdim=True).clamp(min=1e-3)
            path_dir  = path_raw / path_norm
            speed_p   = F.softmax(speed_logits, dim=-1)
            speed_v   = (speed_p * self._decouple_bin_centres).sum(-1, keepdim=True)
            outputs_ego_trajs = path_dir * speed_v * self.speed_dt           # [B, M, T, 2]
            outputs_ego_speed_logits = speed_logits
            # Skip the existing MLP path / decouple block by stuffing outs early.
            outs = {
                'bev_embed': bev_embed,
                ...
                'ego_speed_logits': outputs_ego_speed_logits,
                'ego_feats': ego_feats,
            }
            return outs
        ...
        if self.enable_soft_cmd_routing:
            # single trajectory; keep mode_dim=1 for downstream code that
            # still indexes [B, M, T, 2] (e.g. eval mode-pick falls through
            # naturally because argmax over mode=1 always returns 0).
            outputs_ego_trajs = outputs_ego_trajs.reshape(
                outputs_ego_trajs.shape[0], 1, self.fut_ts, 2)
        else:
            outputs_ego_trajs = outputs_ego_trajs.reshape(
                outputs_ego_trajs.shape[0], self.ego_fut_mode, self.fut_ts, 2)

        # Auxiliary speed-class logits (parallel head)
        outputs_ego_speed_logits = None
        if self.enable_speed_head:
            ego_speed_flat = self.ego_speed_decoder(ego_feats)
            if self.enable_soft_cmd_routing:
                outputs_ego_speed_logits = ego_speed_flat.reshape(
                    ego_speed_flat.shape[0], 1, self.fut_ts, self.num_speed_bins)
            else:
                outputs_ego_speed_logits = ego_speed_flat.reshape(
                    ego_speed_flat.shape[0],
                    self.ego_fut_mode, self.fut_ts, self.num_speed_bins)

            # v9k: v0 skip-channel — add per-step per-bin bias derived from
            # current longitudinal speed (ego_lcf_feat[..., v0_lcf_idx]).
            if self.enable_v0_skip:
                v0 = ego_lcf_feat.squeeze(1).squeeze(1)[..., self.v0_lcf_idx:self.v0_lcf_idx + 1]
                #     [B, 9] → [B, 1]
                v0_bias = self.speed_v0_proj(v0)               # [B, T*K]
                v0_bias = v0_bias.reshape(
                    -1, 1, self.fut_ts, self.num_speed_bins)   # [B, 1, T, K]
                outputs_ego_speed_logits = outputs_ego_speed_logits + v0_bias

        # === v9e: TransFuser++ path / speed decouple ===
        # Reinterpret ego_fut_decoder's output as path direction; combine with
        # softmax-expected-value speed → final per-step offset.
        if self.enable_path_speed_decouple and outputs_ego_speed_logits is not None:
            # Direction: normalise to unit vectors per step
            path_norm = torch.norm(outputs_ego_trajs, dim=-1, keepdim=True).clamp(min=1e-3)
            path_dir = outputs_ego_trajs / path_norm                              # [B, M, T, 2]
            # Speed magnitude: expected value over speed bins (continuous)
            speed_probs = F.softmax(outputs_ego_speed_logits, dim=-1)              # [B, M, T, K]
            speed_value = (speed_probs * self._decouple_bin_centres).sum(
                dim=-1, keepdim=True)                                              # [B, M, T, 1]
            # Compose: per-step displacement = direction × speed × dt
            outputs_ego_trajs = path_dir * speed_value * self.speed_dt

        # === v9j: post-hoc trajectory refinement (forward-only BiGRU) ===
        # Adds residual delta on top of whatever path produced outputs_ego_trajs
        # so far (could be MLP path, decouple-composed, etc.).
        if self.enable_traj_refinement:
            outputs_ego_trajs = self.ego_traj_refiner(outputs_ego_trajs, ego_feats)
        ...
        outs = {
            ...
            'ego_speed_logits': outputs_ego_speed_logits,
            'ego_feats': ego_feats,
            # Exported for downstream language-conditioned agent attention:
            'agent_query': agent_query,         # [B, num_agents, D]
            'agent_pos': agent_pos,             # [B, num_agents, 2]
            'agent_mask': agent_mask,           # [B, num_agents] padding mask (True=padded)
        }
    ...
    def loss_planning(self,
        ...
        # ---- per-step ramp weighting (FDE-style emphasis) ----
        # Linear ramp 1.0 -> plan_step_weight_end over T steps.
        T = ego_fut_preds.shape[-2]
        if self.plan_step_weight_end != 1.0:
            _ramp = torch.linspace(
                1.0, self.plan_step_weight_end, T,
                device=ego_fut_preds.device, dtype=ego_fut_preds.dtype,
            )  # [T]
            _ramp = _ramp[None, None, :, None]   # [1,1,T,1] for broadcast
        else:
            _ramp = None

        # v7 soft routing: ego_fut_preds is single-mode [B, 1, T, 2].
        # Drop cmd-mask weighting + mode-indexing — every sample's GT
        # supervises the (single) prediction.
        if self.enable_soft_cmd_routing:
            # repeat GT to match the singleton mode dim so loss_plan_reg
            # broadcasting stays the same
            ego_fut_gt_rep = ego_fut_gt.unsqueeze(1)             # [B, 1, T, 2]
            loss_plan_l1_weight = ego_fut_masks[:, None, :, None].repeat(1, 1, 1, 2)
            if _ramp is not None:
                loss_plan_l1_weight = loss_plan_l1_weight * _ramp
            picked = ego_fut_preds[:, 0]                          # [B, T, 2]
            loss_plan_l1 = self.loss_plan_reg(
                ego_fut_preds, ego_fut_gt_rep, loss_plan_l1_weight)
            # ---- Explicit FDE term on last step ----
            if self.loss_plan_fde_weight > 0.0:
                fde_w = loss_plan_l1_weight[:, :, -1, :]
                loss_plan_l1 = loss_plan_l1 + self.loss_plan_fde_weight * \
                    self.loss_plan_reg(
                        ego_fut_preds[:, :, -1, :],
                        ego_fut_gt_rep[:, :, -1, :],
                        fde_w)
            loss_plan_bound = self.loss_plan_bound(
                picked, lane_preds, lane_score_preds, weight=ego_fut_masks)
            loss_plan_col = self.loss_plan_col(
                picked, agent_preds, agent_fut_preds, agent_score_preds,
                agent_fut_cls_preds,
    ...
    def loss_speed_classification(self, ego_speed_logits, ego_fut_gt,
                                  ego_fut_masks, ego_fut_cmd):
        """Aux speed-class CE loss for the ego planner.

        Args:
            ego_speed_logits: [B, ego_fut_mode, fut_ts, num_speed_bins]
            ego_fut_gt:       [B, fut_ts, 2]   per-step offsets in ego frame
            ego_fut_masks:    [B, fut_ts]      validity (1 keep)
            ego_fut_cmd:      [B, ego_fut_mode] one-hot

        Returns:
            scalar CE loss(weighted by mask + cmd-selected mode).
        """
        # GT per-step speed (m/s) = ‖offset‖ / dt
        gt_speeds = torch.norm(ego_fut_gt, dim=-1) / max(self.speed_dt, 1e-6)  # [B, T]

        # Bin into class ids using torch.bucketize
        thresholds = torch.tensor(
            self.speed_bins, device=gt_speeds.device, dtype=gt_speeds.dtype)
        gt_classes = torch.bucketize(gt_speeds, thresholds)  # [B, T] long, 0..num_bins-1

        # Pick the (cmd-selected) mode's logits per sample.
        # Soft routing: only one mode exists, just take it.
        B = ego_speed_logits.shape[0]
        if self.enable_soft_cmd_routing:
            picked_logits = ego_speed_logits[:, 0]   # [B, T, num_bins]
        else:
            cmd_idx = ego_fut_cmd.argmax(dim=-1)     # [B]
            picked_logits = ego_speed_logits[torch.arange(B, device=cmd_idx.device),
                                             cmd_idx]  # [B, T, num_bins]

        # CE per (B, T) with mask
        ce = F.cross_entropy(
            picked_logits.reshape(-1, self.num_speed_bins),
            gt_classes.reshape(-1),
            reduction='none',
        ).reshape(B, self.fut_ts)
        mask = ego_fut_masks.float()  # [B, T]
        denom = mask.sum().clamp(min=1.0)
        loss_ce = (ce * mask).sum() / denom

        if digit_version(TORCH_VERSION) >= digit_version('1.8'):
            loss_ce = torch.nan_to_num(loss_ce)
        return loss_ce * self.loss_speed_weight
    ...
    def loss(self,
        ...
        # === Aux speed-class CE loss(只在 enable_speed_head=True 時)===
        ego_speed_logits = preds_dicts.get('ego_speed_logits', None)
        if self.enable_speed_head and ego_speed_logits is not None:
            loss_speed = self.loss_speed_classification(
                ego_speed_logits, ego_fut_gt, ego_fut_masks, ego_fut_cmd,
            )
            loss_dict['loss_ego_speed'] = loss_speed
