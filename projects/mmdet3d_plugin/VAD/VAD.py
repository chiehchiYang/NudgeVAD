...
class VAD(MVXTwoStageDetector):
    ...
    def simple_test_pts(
        ...
        for i, (bboxes, scores, labels, trajs, map_bboxes, \
            ...
            efp_i = outs['ego_fut_preds'][i]
            # v7 soft routing: head emits shape [1, T, 2]. Broadcast to
            # [ego_fut_mode, T, 2] so downstream code that does efp[cmd_idx]
            # keeps working regardless of cmd value.
            if efp_i.shape[0] == 1 and self.pts_bbox_head.ego_fut_mode > 1:
                efp_i = efp_i.expand(self.pts_bbox_head.ego_fut_mode, -1, -1).contiguous()
            bbox_result['ego_fut_preds'] = efp_i.cpu()
            if 'ego_feats' in outs and outs['ego_feats'] is not None:
                bbox_result['ego_feats'] = outs['ego_feats'][i].cpu()
            # Thread agent embeddings to downstream consumers (e.g.
            # TextDeltaPlannerFiLM's agent-aware cross-attention).
            for k in ('agent_query', 'agent_pos', 'agent_mask'):
                if k in outs and outs[k] is not None:
                    bbox_result[k] = outs[k][i].cpu()
            bbox_result['ego_fut_cmd'] = ego_fut_cmd.cpu()
            # Speed-class logits (TransFuser++ aux head). Forwarded so
            # downstream eval can optionally use bin-center speed to
            # rescale the trajectory's per-step magnitude.
            if outs.get('ego_speed_logits') is not None:
                speed_i = outs['ego_speed_logits'][i]
                if speed_i.shape[0] == 1 and self.pts_bbox_head.ego_fut_mode > 1:
                    speed_i = speed_i.expand(
                        self.pts_bbox_head.ego_fut_mode, -1, -1).contiguous()
                bbox_result['ego_speed_logits'] = speed_i.cpu()
